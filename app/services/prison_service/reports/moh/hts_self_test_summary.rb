  ...
  module Reports
    module Moh
      class HtsSelfTestSummary
        ...
        FACILITY = %i[total_recipients_at_the_facility facility_vct facility_anc_first_visit facility_inpatient
                      facility_sti facility_pmtctfup facility_index facility_paediatric facility_vmmc facility_malnutrition facility_tb facility_opd facility_other_pitc facility_sns].freeze
        COMMUNITY = %i[total_recipients_in_the_community community_vmmc community_index community_mobile community_vct
                       community_other community_sns].freeze
        GENDER = %i[female_non_pregnant female_pregnant male].freeze
        SEX = %i[male female].freeze
        AGE_GROUPS = %i[less_than_13 13_to_14 15_to_19 20_to_24 25_to_29 30_to_34 35_to_39 40_to_44 45_to_49
                        50_plus].freeze
        LAST_HIV_TEST = %i[never_tested negative positive_on_art positive_not_on_art inconclusive].freeze
        LAST_SICE_HIV_TEST = %i[twelve_plus_months six_to_eleven_months thirty_five_months fourteen_days_to_two_months
                                one_to_thirteen_days same_day].freeze
        LAST_HIV_RESULT_DATE = %i[not_applicable_or_missing last_hiv_result_same_day last_hiv_result_12_plus_months
                                  last_hiv_result_1_to_13_days last_hiv_result_6_to_11_months last_hiv_result_3_to_5_months last_hiv_result_14_days_to_2_months last_hiv_result_less_than_14_days].freeze
        CONDOMS_GIVEN = %i[condoms_sum].freeze
        ...
        def calc_facility_report(clients)
          ...
          access_type(clients,
                      'Health Facility').where(obs: { concept_id: concept('Location where test took place').concept_id })
            .distinct
        ...
        def calc_community_report(clients)
          ...
          access_type(clients,
                      'Community').where(obs: { concept_id: concept('Location where test took place').concept_id })
            .distinct
            .select('obs.value_text, patient.patient_id')
            .each do |client|
        ...
        def calc_groups(clients)
          ...
          clients
            ...
            SEX.each do |sex|
              report["#{sex}s_less_than_13"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 0, 12
              )
              report["#{sex}s_13_to_14"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 13, 14
              )
              report["#{sex}s_15_to_19"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 15, 19
              )
              report["#{sex}s_20_to_24"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 20, 24
              )
              report["#{sex}s_25_to_29"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 25, 29
              )
              report["#{sex}s_30_to_34"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 30, 34
              )
              report["#{sex}s_35_to_39"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 35, 39
              )
              report["#{sex}s_40_to_44"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 40, 44
              )
              report["#{sex}s_45_to_49"].push(client.id) if client.gender == sex[0].humanize && dob_within(
                client.birthdate, 45, 49
              )
              report["#{sex}s_50_plus"].push(client.id) if client.gender == sex[0].humanize && dob_within(client.birthdate,
                                                                                                          50, 120)
            end
          end
        end
        ...
        def get_diff(obs_time, time_since)
          (time_since&.to_date&.- obs_time&.to_date).to_i
        ...
        def calc_time_since_last_hiv_result(clients)
          ...
          clients.joins(
            <<~SQL
              LEFT JOIN obs time_since on time_since.person_id = encounter.patient_id
              AND time_since.voided = 0
              AND time_since.concept_id = #{ConceptName.find_by_name('Time of HIV test').concept_id}
            SQL
          )
                 .group('patient.patient_id')
                 .select('time_since.value_datetime, obs.obs_datetime, patient.patient_id')
                 .each do |client|
            report[:last_hiv_result_same_day].push(client.patient_id) if get_diff(client.value_datetime,
                                                                                  client.obs_datetime).zero?
            report[:last_hiv_result_12_plus_months].push(client.patient_id) if get_diff(client.value_datetime,
                                                                                        client.obs_datetime) >= 365
            report[:last_hiv_result_6_to_11_months].push(client.patient_id) if (151..330).include?(get_diff(
                                                                                                     client.value_datetime, client.obs_datetime
                                                                                                   ))
            report[:last_hiv_result_3_to_5_months].push(client.patient_id) if (61..150).include?(get_diff(
                                                                                                   client.value_datetime, client.obs_datetime
                                                                                                 ))
            report[:last_hiv_result_14_days_to_2_months].push(client.patient_id) if (14..60).include?(get_diff(
                                                                                                        client.value_datetime, client.obs_datetime
                                                                                                      ))
            report[:last_hiv_result_1_to_13_days].push(client.patient_id) if (1..13).include?(get_diff(
                                                                                                client.value_datetime, client.obs_datetime
                                                                                              ))
            report[:not_applicable_or_missing].push(client.patient_id) if get_diff(client.value_datetime,
                                                                                   client.obs_datetime) == -1
          end
        end
      ...
      INNER JOIN concept_name on concept_name.concept_id = obs.concept_id
          SQL
            ...
            SEX.each do |sex|
              ...
              report["end_user_#{sex}s_less_than_13"] += 1 if age_within(client.value_numeric, 0,
                                                                         12) && gender_is_right(sex,
                                                                                                client.obs_group_id)
              report["end_user_#{sex}s_13_to_14"] += 1 if age_within(client.value_numeric, 13,
                                                                     14) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_15_to_19"] += 1 if age_within(client.value_numeric, 15,
                                                                     19) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_20_to_24"] += 1 if age_within(client.value_numeric, 20,
                                                                     24) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_25_to_29"] += 1 if age_within(client.value_numeric, 25,
                                                                     29) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_30_to_34"] += 1 if age_within(client.value_numeric, 30,
                                                                     34) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_35_to_39"] += 1 if age_within(client.value_numeric, 35,
                                                                     39) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_40_to_44"] += 1 if age_within(client.value_numeric, 40,
                                                                     44) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_45_to_49"] += 1 if age_within(client.value_numeric, 45,
                                                                     49) && gender_is_right(sex, client.obs_group_id)
              report["end_user_#{sex}s_50_plus"] += 1 if age_within(client.value_numeric, 50,
                                                                    120) && gender_is_right(sex, client.obs_group_id)
            end
          end
        end
